                    ...
                    if a ~= pl and not isProxy(a) then
                        ...
                        if dx * dx + dy * dy <= r2 then
                            ...
                            if keepalive or changed(cache[id], p.x, p.y, p.z, yaw, hp) then
                                local ag = aanim[id]
                                net.send(peer, P.ACTOR_STATE, { id = id, rec = a.recordId, x = p.x, y = p.y, z = p.z, yaw = yaw, hp = hp, la = ag and ag.la, ua = ag and ag.ua })
...
eventHandlers = {
    MP_ActorAnim = function(e)
        aanim[e.id] = { la = e.la, ua = e.ua }
        invalidate(e.id)   -- force the next ACTOR_STATE to carry the new groups
    end,
} }
