...
namespace MWNet
{
    ...
    struct EntityState
    {
        ...
        // Present iff this entity is a host-spawned dynamic actor a receiver can't have from shared
        // content (a summon, or any reserved-RefNum spawn — including a save-restored spawn migrated by
        // reconcileLoadedCellActors), carrying the actor record's serialized-text RefId (creature or
        // NPC) so a receiver can instantiate it the first time it sees it (then track and drive it by
        // its RefNum like any other host-owned actor). Re-advertised on full-refresh ticks. Such spawns
        // are host-authoritative: the host owns the actor (a player's summon is routed to the host,
        // which spawns it bound to the summoner's avatar), so its combat and AI ride the normal paths.
